...
def remap(src_file, src_varname, src_grd, dst_grd, dmax=0, cdepth=0, kk=0, dst_dir='./'):
    ...
    if ndim == 3:
        src_var = src_var[:]
        src_var = src_var[:,np.r_[0,:np.size(src_var,1),-1],:]
    elif ndim == 2:
        src_var = src_var[:]
        src_var = src_var[np.r_[0,:np.size(src_var,0),-1],:]
    ...
    if src_varname == 'ssh':
        ...
        wts_file = 'remap_weights_SODA_2.1.6_to_ARCTIC_bilinear_t_to_rho.nc'
        dst_varname = 'zeta'
