...
def remap_uv(src_file, src_grd, dst_grd, dmax=0, cdepth=0, kk=0, dst_dir='./'):
    ...
    src_varu = src_varu[:]
    src_varu = src_varu[:,np.r_[0,:np.size(src_varu,1),-1],:]
    src_varv = src_varv[:]
    src_varv = src_varv[:,np.r_[0,:np.size(src_varv,1),-1],:]

    # get weights file
    wts_file = 'remap_weights_SODA_2.1.6_to_ARCTIC_bilinear_uv_to_rho.nc'
