...
for mm in np.arange(12):
    ...
    for ktr in np.arange(len(list_tracer)):
        ...
        remap_bdry_bio(mydict, src_grd, dst_grd, dst_dir=dst_dir)


    # merge file
    bdry_file = dst_dir + dst_grd.name + '_bdry_bio_GFDL+WOA+GLODAP_m' + str(mm+1).zfill(2) + '.nc' 
    out_file = dst_dir + dst_grd.name + '_bdry_bio_' + list_tracer[0] + '.nc' 
    command = ('ncks', '-a', '-O', out_file, bdry_file) 
    subprocess.check_call(command)
    os.remove(out_file)

    for ktr in np.arange(1,len(list_tracer)):
        out_file = dst_dir + dst_grd.name + '_bdry_bio_' + list_tracer[ktr] + '.nc' 
