...
from remap_bdry_bio import remap_bdry_bio

from remap_bdry_bio_woa import remap_bdry_bio_woa
from remap_bdry_bio_glodap import remap_bdry_bio_glodap

data_dir_woa = '/Volumes/P1/Data/WOA13/interp/CM2.1/'
...
for mm in np.arange(12):
    ...
    for ktr in np.arange(len(list_tracer_update_woa)):
        ...
        'frame':mm}
        remap_bdry_bio_woa(mydict, src_grd, dst_grd, dst_dir=dst_dir)
        out_file = dst_dir + dst_grd.name + '_bdry_bio_' + list_tracer_update_woa[ktr] + '.nc'
...
for mm in np.arange(12):
    ...
    for ktr in np.arange(len(list_tracer_update_glodap)):
        ...
        'frame':mm}
        remap_bdry_bio_glodap(mydict, src_grd, dst_grd, dst_dir=dst_dir)
        out_file = dst_dir + dst_grd.name + '_bdry_bio_' + list_tracer_update_glodap[ktr] + '.nc'
        command = ('ncks', '-a', '-A', out_file, bdry_file)
        subprocess.check_call(command)
        os.remove(out_file)
