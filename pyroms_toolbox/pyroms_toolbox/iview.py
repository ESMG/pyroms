...
def iview(var, tindex, iindex, gridid, filename=None, \
          cmin=None, cmax=None, clev=None, clbformat='%.2f', \
          fill=False, contour=False, d=4, jrange=None, \
    ...
    plot a constante-i slice of variable var. If filename is provided,
    ...
    if fill == True:
        islice, zi, loni, lati, = pyroms.tools.islice(var, iindex, grd, \
                                    Cpos)
    else:
        islice, zi, loni, lati, = pyroms.tools.islice(var, iindex, grd, \
                                    Cpos, vert=True)
    ...
    if fill:
        cf = plt.contourf(lati, zi, islice, vc, cmap = pal, norm = pal_norm, axes=ax)
    else:
    ...
    if map:
        ...
        Basemap.plot(map, x[start[0]:end[0],iindex], y[start[0]:end[0],iindex], \
