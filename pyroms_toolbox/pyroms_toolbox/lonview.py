...
def lonview(var, tindex, longitude, gridid, filename=None, \
            cmin=None, cmax=None, clev=None, fill=False, \
            contour=False, d=4, latrange=None, hrange=None,\
    ...
    if fill == True:
        lonslice, zs, lons, lats, = pyroms.tools.lonslice(var, longitude, grd, Cpos)
    else:
        lonslice, zs, lons, lats, = pyroms.tools.lonslice(var, longitude, grd, Cpos, vert=True)
    ...
    if fill is True:
        cf = plt.contourf(lats, zs, lonslice, vc, cmap = pal, norm = pal_norm, axes=ax)
    else:
        cf = plt.pcolor(lats, zs, lonslice, cmap = pal, norm = pal_norm, axes=ax)
    ...
    if contour is True:
        if fill is not True:
            ...
            plt.contour(lats, zs, lonslice, vc[::d], colors='k', linewidths=0.5, linestyles='solid', axes=ax)
    ...
    if map is True:
        ...
        if latrange is None:
            ...
            c1 = lats[0,:] >= latrange[0]
            c2 = lats[0,:] <= latrange[1]
            c = c1 & c2
            idx = np.where(c == True)
